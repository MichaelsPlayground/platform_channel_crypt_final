...
// based on PullRequest https://github.com/soyersoyer/SwCrypt/pull/51
// version 5.1.4
...
open class CC {
    ...
    open class CCM {
        ...
        public static func crypt(_ opMode: OpMode, algorithm: Algorithm, data: Data,
            ...
            var status: CCCryptorStatus = key.withUnsafeBytes { keyBytes in
                let b0: UnsafePointer<UInt8> = keyBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                return CCCryptorCreateWithMode!(
                    opMode.rawValue, AuthBlockMode.ccm.rawValue,
                    algorithm.rawValue, Padding.noPadding.rawValue,
                    nil, b0, key.count, nil, 0,
            ...
            status = aData.withUnsafeBytes { aDataBytes in
                let b0: UnsafePointer<UInt8> = aDataBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                return CCCryptorAddParameter!(cryptor!, Parameter.authData.rawValue, b0, aData.count)
            }
            guard status == noErr else { throw CCError(status) }
            ...
            status = result.withUnsafeMutableBytes { resultBytes in
                let b0: UnsafeMutablePointer<UInt8> = resultBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                return CCCryptorFinal!(cryptor!, b0 + updateLen,
                                       rescount - updateLen,
                                       &finalLen)
            }
            guard status == noErr else { throw CCError(status) }
            ...
            status = tag.withUnsafeMutableBytes { tagBytes in
                let b0: UnsafeMutablePointer<UInt8> = tagBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
    ...
    open class RSA {
        ...
        fileprivate static func importFromDERKey(_ derKey: Data) throws -> CCRSACryptorRef {
            ...
            let status: CCCryptorStatus = derKey.withUnsafeBytes { derKeyBytes in
                let b0: UnsafePointer<UInt8> = derKeyBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
        ...
        fileprivate static func exportToDERKey(_ key: CCRSACryptorRef) throws -> Data {
            ...
            let status: CCCryptorStatus = derKey.withUnsafeMutableBytes { derKeyBytes in
                let b0: UnsafeMutablePointer<UInt8> = derKeyBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                return CCRSACryptorExport!(key, b0, &derKeyLength)
            }
            guard status == noErr else { throw CCError(status) }
        ...
        fileprivate static func add_pss_padding(_ digest: DigestAlgorithm,
            ...
            maskedDB.withUnsafeMutableBytes { maskedDBBytes in
                let b0: UnsafeMutablePointer<UInt8> = maskedDBBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                b0[0] &= 0xff >> UInt8(zeroBits)
            }
        ...
        fileprivate static func verify_pss_padding(_ digest: DigestAlgorithm,
            ...
            db.withUnsafeMutableBytes { dbBytes in
                let b0: UnsafeMutablePointer<UInt8> = dbBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                b0[0] &= 0xff >> UInt8(zeroBits)
            }
        ...
        open class DH {
            ...
            open func generateKey() throws -> Data {
                ...
                let status: CCCryptorStatus = output.withUnsafeMutableBytes { outputBytes in
                    let b0: UnsafeMutablePointer<UInt8> = outputBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                    return CCDHGenerateKey!(ref!, b0, &outputLength)
                }
    ...
    open class EC {
        ...
        public static func computeSharedSecret(_ privateKey: Data,
            ...
            let status: CCCryptorStatus = result.withUnsafeMutableBytes { resultBytes in
                let b0: UnsafeMutablePointer<UInt8> = resultBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                return CCECCryptorComputeSharedSecret!(privKey, pubKey, b0, &outSize)
            }
            guard status == noErr else { throw CCError(status) }
        ...
        fileprivate static func exportKey(_ key: CCECCryptorRef, format: KeyExternalFormat,
            ...
            let status: CCCryptorStatus = expKey.withUnsafeMutableBytes { expKeyBytes in
                let b0: UnsafeMutablePointer<UInt8> = expKeyBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
    ...
    open class CRC {
        ...
        public static func crc(_ input: Data, mode: Mode) throws -> UInt64 {
            ...
            let status: CCCryptorStatus = input.withUnsafeBytes { inputBytes in
                let b0: UnsafePointer<UInt64> = inputBytes.baseAddress!.assumingMemoryBound(to: UInt64.self)
                return CNCRC!(
                    mode.rawValue,
                    b0, input.count,
                    &result)
    ...
    open class KeyWrap {
        ...
        /* old forced warning Initialization of 'UnsafePointer<UInt8>' results in a dangling pointer
        public static let rfc3394IV = Data(bytes: UnsafePointer<UInt8>(rfc3394IVData), count:rfc3394IVData.count)
        */
        // see https://github.com/soyersoyer/SwCrypt/pull/65/files
...
extension Data {
    ...
    public func hexadecimalString() -> String {
        ...
        self.withUnsafeBytes {data in
            let b0: UnsafePointer<UInt8> = data.baseAddress!.assumingMemoryBound(to: UInt8.self)
            for i in UnsafeBufferPointer<UInt8>(start: b0, count: count) {
    ...
    fileprivate struct BytesView: Collection {
        ...
        subscript (position: Int) -> UInt8 {
            ...
            data.withUnsafeBytes { dataBytes in
                let b0: UnsafePointer<UInt8> = dataBytes.baseAddress!.assumingMemoryBound(to: UInt8.self)
                value = UnsafeBufferPointer<UInt8>(start: b0, count: data.count)[position]
            }
            return value
        }
...
fileprivate func withUnsafePointers<A0, A1, Result>(
    ...
    return try arg0.withUnsafeBytes { p0 in
        ...
        return try arg1.withUnsafeBytes { p1 in
            let b1: UnsafePointer<A1> = p1.baseAddress!.assumingMemoryBound(to: A1.self)
    ...
    return try arg0.withUnsafeBytes { p0 in
        ...
        return try arg1.withUnsafeBytes { p1 in
            ...
            return try arg2.withUnsafeMutableBytes { p2 in
                let b2: UnsafeMutablePointer<A2> = p2.baseAddress!.assumingMemoryBound(to: A2.self)
                return try body(b0, b1, b2)
            }
        }
    }
}
...
fileprivate func withUnsafePointers<A0, A1, A2, Result>(
    ...
    return try arg0.withUnsafeMutableBytes { p0 in
        let b0: UnsafeMutablePointer<A0> = p0.baseAddress!.assumingMemoryBound(to: A0.self)
        return try arg1.withUnsafeMutableBytes { p1 in
            let b1: UnsafeMutablePointer<A1> = p1.baseAddress!.assumingMemoryBound(to: A1.self)
            return try arg2.withUnsafeMutableBytes { p2 in
                let b2: UnsafeMutablePointer<A2> = p2.baseAddress!.assumingMemoryBound(to: A2.self)
    ...
    return try arg0.withUnsafeBytes { p0 in
        ...
        return try arg1.withUnsafeBytes { p1 in
            ...
            return try arg2.withUnsafeBytes { p2 in
                let b2: UnsafePointer<A2> = p2.baseAddress!.assumingMemoryBound(to: A2.self)
                return try arg3.withUnsafeMutableBytes { p3 in
                    let b3: UnsafeMutablePointer<A3> = p3.baseAddress!.assumingMemoryBound(to: A3.self)
                    return try body(b0, b1, b2, b3)
                }
            }
        }
    }
        ...
        return try arg1.withUnsafeBytes { p1 in
            ...
            return try arg2.withUnsafeBytes { p2 in
                ...
                return try arg3.withUnsafeBytes { p3 in
                    let b3: UnsafePointer<A3> = p3.baseAddress!.assumingMemoryBound(to: A3.self)
                    return try arg4.withUnsafeMutableBytes { p4 in
                        let b4: UnsafeMutablePointer<A4> = p4.baseAddress!.assumingMemoryBound(to: A4.self)
                        return try arg5.withUnsafeMutableBytes { p5 in
                            let b5: UnsafeMutablePointer<A5> = p5.baseAddress!.assumingMemoryBound(to: A5.self)
                            return try body(b0, b1, b2, b3, b4, b5)
                        }
                    }
                }
            }
